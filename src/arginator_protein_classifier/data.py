...
class MyDataset(Dataset):
    ...
    def __init__(
        self, data_path: Path, output_folder: Path = None, force_process: bool = False, task: str = "Binary"
    ) -> None:
...
def get_dataloaders(data_path, task: str = "Binary", batch_size=32, split_ratios: list = None, seed: int = 42):
    ...
    train_loader = DataLoader(train_subset, batch_size=batch_size, sampler=train_sampler, num_workers=2)
...
class TL_Dataset(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        val_idx, test_idx = train_test_split(
            temp_idx, test_size=(1 - val_ratio_adjusted), stratify=temp_labels, random_state=self.seed
        )
    ...
    def train_dataloader(self):
        return DataLoader(self.train_subset, batch_size=self.batch_size, sampler=self.train_sampler, num_workers=2)
