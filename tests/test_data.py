...
def create_dummy_h5(folder, filename, num_samples=10):
    ...
    This matches the logic in your data.py: 'for key in hf.keys(): ...'
    """
    filepath = folder / filename
    
    with h5py.File(filepath, "w") as f:
        for i in range(num_samples):
            # Create a random 1D vector representing one protein embedding
            # Shape is (1024,) to match model input
            data = np.random.randn(1024).astype('float32')
            
            # The key name is not relevant just needs to be unique
...
def test_get_dataloaders(tmp_path):
    ...
    # We inspect the full dataset to be sure
