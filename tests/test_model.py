...
def model(hydra_mocks):
    ...
    return Lightning_Model(
        input_dim=INPUT_DIM, output_dim=OUTPUT_DIM, dropout_rate=0.1, loss_fn=loss_cfg, optimizer_config=opt_cfg
    )
...
def test_model_forward_shape(model, batch_size):
    ...
    # Assertions
    assert y.shape == (batch_size, OUTPUT_DIM), f"Expected output shape {(batch_size, OUTPUT_DIM)}, got {y.shape}"
...
def test_model_initialization(dropout, hydra_mocks):
    ...
    model = Lightning_Model(
        input_dim=INPUT_DIM, output_dim=OUTPUT_DIM, dropout_rate=dropout, loss_fn=loss_cfg, optimizer_config=opt_cfg
    )
